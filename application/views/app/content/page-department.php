...
<script>
	...
	pageDepartment.init('#pageDepartment', function(result){
		...
		// 	trDepartment.attr('class', key);
		// 	trDepartment.find('td').attr('class', "column" +key);
		// 	trDepartment.find('td').html(value['dept_name']);
		// 	pageDepartment.elem.find('.department-table tbody').append(trDepartment);

		// 	pageDepartment.tdEdit = $('<td><a></a></td>');
		// 	pageDepartment.tdEdit.find('a').attr('class', 'icon icon-left s7-note text-success');
			// pageDepartment.tdEdit.find('a').off("click").click(function(e){
			// 	pageDepartment.elem.find('.btn-save').removeClass('btn-dept-save').addClass('btn-dept-edit');
			// 	pageDepartment.elem.find('.modal-title').text('Edit Department');
			// 	pageDepartment.elem.find('.dept_name').val(value['dept_name']);
			// 	pageDepartment.elem.find('.modal-edit-department').modal();
			...
			// 		$.ajax({
			// 			method: "POST",
			// 			url: "<?php echo base_url('update-department/')?>" + value['dept_Id'],
			// 			data: pageDepartment.editedName,
			// 			success: function(result){
			// 				if(result.success){
			// 					pageDepartment.elem.find('.i-circle').removeClass('text-danger').addClass('text-success');
			...
			// 	});
			// });
		// 	pageDepartment.elem.find('.'+key).append(pageDepartment.tdEdit);

		// 	pageDepartment.tdDelete = $('<td><a></a></td>');
		// 	pageDepartment.tdDelete.find('a').attr('class', 'icon icon-left s7-trash text-danger');
		// 	pageDepartment.tdDelete.find('a').off("click").click(function(e){
		// 		pageDepartment.elem.find('.i-circle').removeClass('text-success').addClass('text-danger');
		// 		pageDepartment.elem.find('.symbol').removeClass('s7-check').addClass('s7-attention');
		// 		pageDepartment.elem.find('.message').html('Are you sure you want to delete this data!');
		// 		pageDepartment.elem.find('.btn-proceed').hide();
		...
		// 			$.ajax({
		// 				method: "POST",
		// 				url: "<?php echo base_url('delete-department/')?>" + value['dept_Id'],
		// 				success: function(result){
		// 					if(result.success){
		// 						pageDepartment.elem.find('.i-circle').removeClass('text-danger').addClass('text-success');
		// 						pageDepartment.elem.find('.symbol').removeClass('s7-attention').addClass('s7-check');
		...
		// 	});
		// 	pageDepartment.elem.find('.'+key).append(pageDepartment.tdDelete);
		// });
